...
3. **Choose a tag**를 클릭하고 새 버전을 입력(예: `v1.0-beta`)한 뒤 **Create new tag**를 클릭합니다.
   - 태그명은 반드시 `v`로 시작해야 합니다 (예: `v1.0-beta` O, `1.0-beta` X).
   - 기존에 동일한 태그명이 원격 서버에 이미 등록되어 있다면 빌드가 무시되거나 트리거되지 않으므로, 충돌 시 기존 원격 태그를 깃허브 웹(또는 `git push origin :refs/tags/v1.0-beta` 명령어)으로 삭제하고 릴리즈를 새로 만드셔야 합니다.
