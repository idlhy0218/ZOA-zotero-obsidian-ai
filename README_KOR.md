# ZOA (Zotero-Obsidian-AI Summary) v1.0.4

<p align="center" bgcolor="white">
<img width="2000" height="1990" alt="zoa icon github readme" src="https://github.com/user-attachments/assets/5d79fdfa-0dd3-4d62-88c2-be76b7e6da5b" />
</p>
...
- **v1.0.4: UI/UX 대폭 개선, 실시간 진행 정보 카드 및 Zotero 동기화 가이드 강화 (2026-09-23)**:
  - **헤더 구성 요약 뱃지 바**: 현재 선택된 AI 제공자, 모델명, Zotero 노트 동기화 활성화 여부를 상단 헤더에서 한눈에 직관적으로 확인할 수 있는 뱃지 UI 추가.
  - **컬렉션 자동 로딩 및 빈 상태 안내(Empty State)**: 앱 실행 시 유효한 Zotero 경로가 설정되어 있으면 컬렉션을 자동으로 로드하며, 미선택 시 직관적인 안내 카드를 표시.
  - **실시간 처리 현황 카드 (Section 03)**: 논문 처리 중 현재 분석 중인 논문 제목, 세부 처리 단계(추출, 프롬프트 전송, 마크다운 저장, Zotero 싱크), 완료/스킵/잔여 수량 통계를 실시간으로 안내.
  - **Zotero Cloud 동기화 알림 강화**: Zotero Web API로 작성된 하위 노트가 데스크톱 앱에 즉각 반영되지 않을 경우 동기화 단축키(`Ctrl+Shift+S`) 안내 및 클라이언트 재실행 팁을 실행 로그와 툴팁에 명시.
